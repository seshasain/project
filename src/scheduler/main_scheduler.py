...
class SerialProcessor:
        ...
        try:
            if os.path.exists(self.tracking_file):
                with open(self.tracking_file, 'r') as f:
                    ...
                    logger.info(f"Loaded {len(self.processed_serials)} processed serials for today (IST)")
            else:
                self.processed_serials = set()
                logger.info("No processed serials found for today (IST)")
        except Exception as e:
    ...
    def save_processed_serials(self):
        ...
        try:
            ...
            logger.info(f"Successfully saved {len(self.processed_serials)} processed serials (IST)")
        except Exception as e:
    ...
    def process_serial(self, serial_name):
        ...
        if serial_name in self.processed_serials:
            logger.info(f"{serial_name} already processed today (IST)")
            return False
    ...
    def check_serials(self):
        ...
        try:
            ...
            # Only run between 8 AM and 3 PM IST
            if not (8 <= current_hour < 15):
                logger.info("Outside operating hours (8 AM - 3 PM IST)")
                return

            if len(self.processed_serials) == len(self.target_serials):
                logger.info("All serials processed for today (IST)")
                return
